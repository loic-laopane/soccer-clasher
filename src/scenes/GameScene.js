class GameScene extends Phaser.Scene {
  ...
  _buildField() {
    ...
    // Crowd
    const crowdColors = [0xCC2200,0xFF4400,0xFFAA00,0x0044CC,0x8800CC,0xFFFFFF,0xFF8800,0x00AA44];
    ...
    for (let row=0;row<5;row++) for (let i=0;i<Math.floor(H/9);i++) {
      g.fillStyle(crowdColors[Math.floor(rn()*crowdColors.length)],0.85);
      g.fillCircle(1+row*2.5+rn(),5+i*9+rn()*3,2.2);
    }
    for (let row=0;row<5;row++) for (let i=0;i<Math.floor(H/9);i++) {
      g.fillStyle(crowdColors[Math.floor(rn()*crowdColors.length)],0.85);
      g.fillCircle(W-1-row*2.5-rn(),5+i*9+rn()*3,2.2);
    }
    for (let col=0;col<5;col++) for (let i=0;i<Math.floor(W/9);i++) {
      g.fillStyle(crowdColors[Math.floor(rn()*crowdColors.length)],0.85);
      g.fillCircle(5+i*9+rn()*3,1+col*1.8+rn(),2.2);
    }
    for (let col=0;col<5;col++) for (let i=0;i<Math.floor(W/9);i++) {
      g.fillStyle(crowdColors[Math.floor(rn()*crowdColors.length)],0.85);
      g.fillCircle(5+i*9+rn()*3,H-1-col*1.8-rn(),2.2);
    }
    ...
    const penW=200,penH=90,penLX=(W-penW)/2;
    const gaW=100,gaH=40,gaLX=(W-gaW)/2;
    ...
    g.beginPath(); g.arc(W/2,H-10-60,44,Phaser.Math.DegToRad(205),Phaser.Math.DegToRad(335),false); g.strokePath();
    ...
    // Bridges (decorative — movement no longer restricted)
    const BLX=CONFIG.BRIDGE_LEFT_X, BRX=CONFIG.BRIDGE_RIGHT_X, BW=CONFIG.BRIDGE_W, CY=CONFIG.CENTER_Y;
    g.fillStyle(0xC8A050,0.45);
    g.fillRect(BLX-BW/2,CY-8,BW,16); g.fillRect(BRX-BW/2,CY-8,BW,16);
    g.lineStyle(1.5,0xFFD700,0.8);
    g.strokeRect(BLX-BW/2,CY-8,BW,16); g.strokeRect(BRX-BW/2,CY-8,BW,16);
    this.add.text(BLX,CY-2,'⇅',{fontSize:'12px',color:'#FFD700',fontFamily:'Arial'}).setOrigin(0.5).setDepth(4).setAlpha(0.8);
    this.add.text(BRX,CY-2,'⇅',{fontSize:'12px',color:'#FFD700',fontFamily:'Arial'}).setOrigin(0.5).setDepth(4).setAlpha(0.8);
  ...
  _buildUI() {
    ...
    this.add.text(W/2,UY-14,this.playerTeam.shortName,{fontSize:'12px',color:'#FFFFFF',fontFamily:'Arial',fontStyle:'bold',stroke:'#000000',strokeThickness:3}).setOrigin(0.5,1).setDepth(15);
    ...
    this.add.text(barX,UY+8,'ÉNERGIE',{fontSize:'10px',color:'#FFD700',fontFamily:'Arial',fontStyle:'bold'}).setDepth(15);
    ...
    this._energyText=this.add.text(W-8,barY,'5',{fontSize:'12px',color:'#FFFFFF',fontFamily:'Arial',fontStyle:'bold'}).setOrigin(1,0.5).setDepth(17);

    this.instrText=this.add.text(W/2,UY+42,'Glissez une carte sur le terrain',{fontSize:'11px',color:'#AAAAAA',fontFamily:'Arial'}).setOrigin(0.5).setDepth(15);
  }
  ...
  _createDragGhost(card, x, y) {
    ...
    // Drop ring indicator
    ghost.ring = this.add.circle(x, y, 22, 0x44AAFF, 0.35).setDepth(48)
      .setStrokeStyle(2, 0x44AAFF, 0.9);

    // Character body + head (semi-transparent)
    ...
    ghost.objs = [ghost.ring, body, stripe, head];
    ghost._offY = sz.bh/2 + sz.hr + 5; // offset so ghost appears above finger
    ghost._sz = sz;
  ...
  _moveDragGhost(px, py) {
    ...
    const y  = py - g._offY;  // show above finger
    const sz = g._sz;
    const headY = y - sz.bh/2 - sz.hr;

    const valid = this._isValidDrop(px, py);
    const ringColor = valid ? 0x44FF88 : 0xFF4444;
    g.ring.setPosition(px, y).setFillStyle(ringColor, 0.25).setStrokeStyle(2,ringColor,0.9);
  ...
  _destroyDragGhost() {
    if (this._dragGhost) { this._dragGhost.objs.forEach(o=>o.destroy()); this._dragGhost=null; }
  }
  ...
  _renderCards() {
    ...
    this.cardObjects=[];
    const W=CONFIG.WIDTH, cw=90, ch=116, gap=6;
    ...
    for(let i=0;i<4;i++){
      ...
      const bg=this.add.rectangle(cx,cy,cw,ch,0x1a1a3e)
        ...
        .setDepth(15).setInteractive({useHandCursor:true,draggable:false});

      // Drag: pointerdown starts drag
      ...
      // Team color strip
      group.push(this.add.rectangle(cx,cy-ch/2+9,cw,18,parseInt(this.playerTeam.jerseyColor.slice(1),16))
        .setDepth(16).setAlpha(canAfford?1:0.4));
      group.push(this.add.text(cx,cy-ch/2+9,card.position,{fontSize:'9px',color:'#FFD700',fontFamily:'Arial',fontStyle:'bold'})
        .setOrigin(0.5).setDepth(17).setAlpha(canAfford?1:0.5));

      // Avatar
      const sz=ROLE_SIZE[card.position]??{bw:16,bh:14,hr:8}, sc=0.78;
      const jc=parseInt(this.playerTeam.jerseyColor.slice(1),16);
      group.push(
        this.add.rectangle(cx,cy-20,sz.bw*sc,sz.bh*sc,jc).setStrokeStyle(1,0x000000).setDepth(17).setAlpha(canAfford?1:0.5),
        this.add.circle(cx,cy-20-sz.bh*sc/2-sz.hr*sc,sz.hr*sc,0xFFCCAA).setStrokeStyle(1,0x000000).setDepth(18).setAlpha(canAfford?1:0.5),
      );

      group.push(this.add.text(cx,cy+8,card.name,{fontSize:'9px',color:canAfford?'#FFFFFF':'#555577',fontFamily:'Arial',fontStyle:'bold',wordWrap:{width:cw-6},align:'center'}).setOrigin(0.5).setDepth(17));
      group.push(this.add.text(cx,cy+26,card.abDesc,{fontSize:'8px',color:canAfford?'#88AAFF':'#333355',fontFamily:'Arial',align:'center',wordWrap:{width:cw-6}}).setOrigin(0.5).setDepth(17));

      group.push(
        this.add.circle(cx+cw/2-12,cy+ch/2-12,13,0x7700CC).setStrokeStyle(1,0xFFFFFF).setDepth(17).setAlpha(canAfford?1:0.5),
        this.add.text(cx+cw/2-12,cy+ch/2-12,card.cost.toString(),{fontSize:'12px',color:'#FFFFFF',fontFamily:'Arial',fontStyle:'bold'}).setOrigin(0.5).setDepth(18).setAlpha(canAfford?1:0.5),
      );
  ...
  _setupInput() {
    ...
    this.input.on('pointerup', ptr => {
      ...
      if (this._isValidDrop(ptr.x, ptr.y) && !this.gameOver) {
        ...
        const y = Phaser.Math.Clamp(ptr.y - (this._dragGhost?._offY??0),
          CONFIG.CENTER_Y+20, CONFIG.FIELD_BOTTOM-20);
